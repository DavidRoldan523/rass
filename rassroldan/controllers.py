...
class ReportController(http.Controller):
    ...
    def rass(self, **args):
        ...
        if dini:
            ...
            if validate == True: 
                ...
                if not dini:
                    ...
                    company = http.request.env["report_rass.configuration"].sudo().search([('BusinessId','=',dini["cabecera"]["IdEmpresa"]), ('InvoiceType','=', dini["cabecera"]["BillType"])] ,limit=1)
                    #bill = http.request.env["report_rass.configuration"].sudo().search([('InvoiceType','=',dini["cabecera"]["BillType"])]))
                    if not company:
                        return json.dumps({
                            "error": "Esta empresa no esta configurada."
                        })                                          
                    else:
                        _logger.info('company type %s', company.InvoiceType)                    
                        try:
                            POLLING_DELAY = 1.00
                            rassobj = self.saveModelFromDini(dini)
                            #pepe = http.request.env['report_Srass.dini'].sudo().browse(rassobj.id)                    
        ...
        #return request.session.uid
            ...
            return json.dumps({
                                "userMessage": "La peticion es incorrecta, no se recibieron los parametros"
                            })
            ...
            try:
                ...
                if dini:
                    ...
                    rassobj = diniobj.sudo().create({
                        ...
                        'BillType': dini["cabecera"]["BillType"],
